define(["leaflet", "rbush", "helper", "moment"], function (L, RBush, helper, moment) {
  "use strict";
  ...
  function addLinksToMap(dict, linkScale, graph) {
    ...
    return graph.map(function (d) {
      ...
      line.bindTooltip(
        helper.escape(d.source.hostname + " – " + d.target.hostname) +
          "<br><strong>" +
          helper.showDistance(d) +
          " / " +
          helper.showTq(d.source_tq) +
          " - " +
          helper.showTq(d.target_tq) +
          "<br>" +
          d.type +
          "</strong>",
      );
  ...
  return L.GridLayer.extend({
    ...
    setData: function (data, map, nodeDict, linkDict, linkScale) {
      ...
      var markersOnline = nodesOnline.map(
        mkMarker(nodeDict, function (d) {
          if (helper.hasUplink(d)) {
