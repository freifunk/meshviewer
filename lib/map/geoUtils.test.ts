...
describe("geoUtils", () => {
  ...
  describe("loadGeoLayer", () => {
    ...
    it("ignores a string json value instead of treating it as a URL", async () => {
      const addToMap = vi.fn();
      const mockFetch = vi.fn();

      const geo = { json: "/map/test-string-url.geojson" } as unknown as Geo;
    ...
    it("reports fetch failures through onError", async () => {
      ...
      const geo: Geo = {
        url: "/map/forbidden.geojson",
      };

      await loadGeoLayer(geo, addToMap, mockFetch as any, onError);
      expect(onError).toHaveBeenCalledTimes(1);
      expect(onError).toHaveBeenCalledWith("Failed to load GeoJSON from /map/forbidden.geojson:", expect.any(Error));
      expect((onError.mock.calls[0]![1] as Error).message).toBe("HTTP 403 Forbidden");
      consoleSpy.mockRestore();
    });

    it("reports inline parse failures through onError", async () => {
      const addToMap = vi.fn();
      const onError = vi.fn();
      const consoleSpy = vi.spyOn(console, "error").mockImplementation(() => {});
      const mockedGeoJSON = vi.mocked(L.geoJSON).mockImplementationOnce(() => {
      ...
      await loadGeoLayer(geo, addToMap, undefined, onError);
      expect(onError).toHaveBeenCalledWith("Failed to parse inline GeoJSON:", expect.any(Error));
      consoleSpy.mockRestore();
      mockedGeoJSON.mockRestore();
    });

    it("does not call onError for aborted requests", async () => {
      const addToMap = vi.fn();
      const onError = vi.fn();
      ...
      await loadGeoLayer({ url: "/map/aborted.geojson" }, addToMap, mockFetch as any, onError, controller.signal);
    ...
    it("passes AbortSignal to fetch and ignores aborted requests cleanly", async () => {
      ...
      const result = await loadGeoLayer(geo, addToMap, mockFetch as any, undefined, controller.signal);
  ...
  describe("loadGeoLayers", () => {
    ...
    it("forwards AbortSignal to each layer load", async () => {
      ...
      await loadGeoLayers(geoList, addToMap, mockFetch as any, undefined, controller.signal);
    ...
    it("forwards onError to each layer load", async () => {
      const addToMap = vi.fn();
      const onError = vi.fn();
      const consoleSpy = vi.spyOn(console, "error").mockImplementation(() => {});
      const mockFetch = vi.fn().mockRejectedValue(new Error("Network connection lost"));

      const geoList: Geo[] = [{ url: "/map/a.geojson" }, { url: "/map/b.geojson" }];

      await loadGeoLayers(geoList, addToMap, mockFetch as any, onError);
