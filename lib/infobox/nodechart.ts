...
import { Node as NodeData } from "../utils/node.js";
import { NodeChart, NodeChartSeries } from "../config_default.js";
...
async function fetchChartData(grafanaUrl: string, orgId: number, nodeChart: NodeChart, nodeId: string): Promise<any> {
  const fromStr = nodeChart.from ?? "now-7d";
  const toStr = nodeChart.to ?? "now-1m";
  const maxDataPoints = nodeChart.maxDataPoints ?? 300;
  ...
  if (nodeChart.datasourceType === "fixture") {
    const response = await fetch(`./grafana-query.json`);
    if (!response.ok) throw new Error(`HTTP ${response.status}`);
    return response.json();
  }

  const query = nodeChart.query.replace(/\$node\b/g, nodeId);

  const response = await fetch(`${grafanaUrl}/api/ds/query?ds_type=${encodeURIComponent(nodeChart.datasourceType)}`, {
    method: "POST",
    mode: "cors",
    credentials: "omit",
    headers: {
      "content-type": "application/json",
      "x-datasource-uid": nodeChart.datasourceUid,
      "x-grafana-org-id": String(orgId),
    },
    body: JSON.stringify({
      queries: [
        {
          datasource: { type: nodeChart.datasourceType, uid: nodeChart.datasourceUid },
          rawQuery: true,
          query,
...
function parseResults(results: Record<string, any>, configMap: Map<string, NodeChartSeries>): ParsedChart {
  const series: Series[] = [];
  let tMin = Infinity,
    tMax = -Infinity;
  let vMin = Infinity,
...
function renderD3Chart(
  ...
  spec: string,
  suffix: string,
  configMap: Map<string, NodeChartSeries> = new Map(),
) {
...
export function createNodeChartVNode(nodeChart: NodeChart, node: NodeData): VNode {
  const grafana = window.config.grafana;
  if (!grafana) return h("div");
  ...
  return h("div", {
    ...
    hook: {
      insert: async (vnode: VNode) => {
        ...
        el.textContent = _.t("loading", { name: nodeChart.name });
        try {
          const json = await fetchChartData(grafanaUrl, orgId, nodeChart, node.node_id);
          el.textContent = "";
          const configMap = new Map((nodeChart.series ?? []).map((s) => [s.name, s]));
          const parsed = parseResults(json.results ?? {}, configMap);
          if (parsed.series.length === 0 || !Number.isFinite(parsed.xDomain[0])) {
            el.textContent = _.t("node.chartNoData", { name: nodeChart.name });
            return;
          }
          renderD3Chart(el, parsed, nodeChart.format ?? ".2~s", nodeChart.unitSuffix ?? "", configMap);
        } catch {
          el.textContent = _.t("node.chartError", { name: nodeChart.name });
        }
      },
    },
  });
