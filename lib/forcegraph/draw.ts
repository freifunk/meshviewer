...
const self = {
  drawNode(node: MapNode) {
    ...
    if (node.o.is_online) {
      ...
      if (helper.hasUplink(node.o)) {
        ctx.fillStyle = config.forceGraph.nodeUplinkColor ?? "#000";
      } else {
        ctx.fillStyle = config.forceGraph.nodeColor ?? "#000";
      }
    } else {
  ...
  drawLink(link: MapLink) {
    ...
    const grd = ctx.createLinearGradient(link.source.x, link.source.y, link.target.x, link.target.y);
  ...
  setCTX(newValue: CanvasRenderingContext2D) {
    ctx = newValue;
  },

  setHighlight(newValue: Highlight) {
    highlight = newValue;
  },

  setTransform(newValue: ZoomTransform) {
    transform = newValue;
  },

  setMaxArea(newWidth: number, newHeight: number) {
    width = newWidth;
    height = newHeight;
  },
};
