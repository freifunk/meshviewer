...
self.drawNode = function drawNode(d) {
  if (
    d.x < transform.invertX(0) ||
    d.y < transform.invertY(0) ||
    transform.invertX(width) < d.x ||
    transform.invertY(height) < d.y
  ) {
    return;
  }
  ctx.beginPath();
...
self.drawLink = function drawLink(d) {
  ...
  if (
    (d.source.x < zero[0] && d.target.x < zero[0]) ||
    (d.source.y < zero[1] && d.target.y < zero[1]) ||
    (d.source.x > area[0] && d.target.x > area[0]) ||
    (d.source.y > area[1] && d.target.y > area[1])
  ) {
    return;
  }
  ctx.beginPath();
